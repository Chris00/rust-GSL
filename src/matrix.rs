    ...
    ($rust_name:ident, $name:ident, $rust_ty:ident, $vec_name:ident, $vec_c_name:ident) => {
        paste! {
            ...
            impl $rust_name {
                ...
                /// Return a matrix view of `base`.  The matrix has `n1` rows
                /// and `n2` columns.  The (`i`, `j`)-th element of the new matrix
                /// is given by
                ///
                /// ```text
                /// ext m'(i,j) = base[i*n2 + j]
                /// ```
                ///
                /// where the index `i` runs from `0` to `n1-1` and the index
                /// `j` runs from `0` to `n2-1`.
                ///
                /// Panic if `n1` * `n2` is larger than the length of `base`.
                // TODO: The function gsl_matrix_const_view_array is equivalent to
                // gsl_matrix_view_array but can be used for matrices which
                // are declared const.
                ...
                pub fn from_mut_slice(
                    ...
                    if n1 * n2 > base.len() {
                        panic!("rgsl::matrix::from_mut_slice: n1 * n2 = {n1} * {n2} > slice length = {}",
                            base.len())
                    }
                ...
                /// This function returns the (i,j)-th element of the matrix.  If
                /// y or x lie outside the allowed range of 0 to n1-1 and 0 to
                ...
                /// This function sets the value of the (i,j)-th element of the
                /// matrix to value.  If y or x lies outside the allowed range of
                ...
                /// Set the elements of the matrix to the corresponding elements
                /// of the identity matrix, m(i,j) = \delta(i,j), i.e. a unit
                /// diagonal with all off-diagonal elements zero.  This applies to
                ...
                /// Copy the elements of the other matrix into the self
                /// matrix.  The two matrices must have the same size.
                ...
                /// Copy the elements of the y-th row of the
                ...
                pub fn get_row(&self, y: usize) -> Result<$vec_name, Error> {
                    ...
                    if tmp.is_null() {
                        ...
                        let ret = unsafe { sys::[<$name _get_row>](
                            tmp, self.unwrap_shared(), y) };

                        Error::handle(ret, ffi::FFI::wrap(tmp))
                    }
                }

                /// Copu the elements of the `x`-th column of the matrix into
                /// the returned vector.
                ...
                pub fn get_col(&self, x: usize) -> Result<$vec_name, Error> {
                    ...
                    if tmp.is_null() {
                        ...
                        let ret = unsafe { sys::[<$name _get_col>](
                            tmp, self.unwrap_shared(), x) };

                        Error::handle(ret, ffi::FFI::wrap(tmp))
                    }
                }

                /// Copy the elements of the vector v into the y-th row of the
                /// matrix.  The length of the vector must be the same as the
                /// length of the row.
                ...
                pub fn set_row(
                    ...
                    let ret = unsafe { sys::[<$name _set_row>](
                        self.unwrap_unique(), y, v.unwrap_shared()) };
                    Error::handle(ret, ())
                }

                /// Copy the elements of the vector v into the x-th column of
                /// the matrix.  The length of the vector must be the same as the
                ...
                pub fn set_col(
                    ...
                    let ret = unsafe { sys::[<$name _set_col>](
                        self.unwrap_unique(), x, v.unwrap_shared()) };
                    Error::handle(ret, ())
                }

                /// Exchange the y1-th and y2-th rows of the matrix in-place.
                #[doc(alias = $name _swap_rows)]
                pub fn swap_rows(&mut self, y1: usize, y2: usize) -> Result<(), Error> {
                    let ret = unsafe { sys::[<$name _swap_rows>](
                        self.unwrap_unique(), y1, y2) };
                    Error::handle(ret, ())
                }

                /// Exchange the x1-th and x2-th columns of the matrix in-place.
                ...
                pub fn swap_columns(
                    ...
                    let ret = unsafe { sys::[<$name _swap_columns>](
                        self.unwrap_unique(), x1, x2) };
                    Error::handle(ret, ())
                }

                /// Exchange the i-th row and j-th column of the matrix in-place.
                ...
                /// Return the transpose of the matrix by copying the elements
                /// into it.  This function works for all matrices provided that
                /// the dimensions of the matrix dest match the transposed
                ...
                /// Add the elements of the other matrix to the elements of
                /// the self matrix.  The result self(i,j) <- self(i,j) +
                /// other(i,j) is stored in self and other remains unchanged. The
                ...
                /// Subtract the elements of the other matrix from the
                /// elements of the self matrix. The result self(i,j) <- self(i,j)
                /// - other(i,j) is stored in self and other remains
                ...
                /// Multiply the elements of the self matrix by the elements
                /// of the other matrix.  The result self(i,j) <- self(i,j) *
                /// other(i,j) is stored in self and other remains
                /// unchanged. The two matrices must have the same dimensions.
                ...
                /// Divide the elements of the self matrix by the elements of
                /// the other matrix.  The result self(i,j) <- self(i,j) /
                /// other(i,j) is stored in self and other remains unchanged. The
                /// two matrices must have the same dimensions.
                ...
                /// Multiply the elements of the self matrix by the constant
                /// factor x. The result self(i,j) <- x self(i,j) is stored in
                /// self.
                ...
                /// matrix. The result self(i,j) <- self(i,j) + x is stored in
                /// self.
                ...
                /// Return the maximum value in the self matrix.
                #[doc(alias = $name _max)]
                pub fn max(&self) -> $rust_ty {
                    unsafe { sys::[<$name _max>](self.unwrap_shared()) }
                }

                /// Return the minimum value in the self matrix.
                #[doc(alias = $name _min)]
                pub fn min(&self) -> $rust_ty {
                    unsafe { sys::[<$name _min>](self.unwrap_shared()) }
                }

                /// Return the minimum and maximum values in the self matrix.
                ...
                /// Return the indices of the maximum value in the self
                /// matrix. When there are several equal maximum elements then
                /// the first element found is returned, searching in
                /// row-major order.
                ...
                /// Return the indices of the minimum value in the self
                /// matrix. When there are several equal minimum elements then the
                ...
                /// Return the indices of the minimum and maximum values in the
                /// self matrix. When there are several equal minimum or maximum
                /// elements then the first elements found are returned, searching
                /// in row-major order.
                ...
                /// Return `true` if all the elements of the self matrix are zero.
                ...
                /// Return a matrix view of a submatrix.  The upper-left
                /// element of the submatrix is the element (`k1`, `k2`) of
                /// the original matrix.  The submatrix has `n1` rows and `n2`
                /// columns.
                ///
                /// Panic if the combined parameters (`k1`, `k2`, `n1`, `n2`)
                ...
                pub fn submatrix(
                    ...
                    unsafe {
                        let view = sys::[<$name _submatrix>](self.mat, k1, k2, n1, n2);
                        if view.matrix.data.is_null() {
                            panic!("rgls::matrix::submatrix: (k1, k2, n1, n2) = ({k1}, {k2}, {n1}, {n2}) exceeds the matrix size ({}, {})",
                ...
                pub fn clone(&self) -> Result<Self, Error> {
                    ...
                    let mut m = Self::new(self.size1(), self.size2());
                    m.copy_from(self)?;
                    Ok(m)
                }
