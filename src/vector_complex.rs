    ...
    ($rust_name:ident, $name:ident, $complex:ident, $rust_ty:ident) => {
        paste! {
            ...
            impl $rust_name {
                ...
                pub fn from_slice(slice: &[$complex]) -> Self {
                ...
                pub(crate) unsafe fn view_mut<'a>(
                    ...
                    ViewMut::alloc("vector_complex::view_mut", view.vector)
                }
                ...
                /// Return the i-th element of a vector v. If i lies outside
                /// the allowed range of 0 to n-1 then the error handler
                /// is invoked and 0 is returned.
                #[doc(alias = $name _get)]
                pub fn get(&self, i: usize) -> $complex {
                    unsafe { sys::[<$name _get>](self.unwrap_shared(), i).wrap() }
                }

                /// Set the value of the i-th element of a vector v to x. If i
                /// lies outside the allowed range of 0 to n-1 then the
                ...
                /// Set all the elements of the vector to the value `x`.
                ...
                /// Set all the elements of the vector v to zero.
                ...
                /// vector v to zero except for the i-th element which is
                /// set to one.
                ...
                /// Exchange the i-th and j-th elements of the vector v in-place.
                ...
                /// Reverse the order of the elements of the vector v.
                ...
                /// the `self` vector.  The result a_i <- a_i + b_i is
                ...
                /// Subtract the elements of the self vector from the elements
                    /// of // TODO: he other vector. The result a_i <- a_i - b_i is
                /// stored in self and other remains unchanged. The two
                /// vectors must have the same length.
                #[doc(alias = $name _sub)]
                ...
                /// Multiply the elements of the self vector a by the elements
                ...
                /// Divide the elements of the self vector by the elements of
                /// the other vector.  The result a_i <- a_i / b_i is
                /// stored in self and other remains unchanged. The two
                /// vectors must have the same length.
                #[doc(alias = $name _div)]
                ...
                /// factor `x`.  The result a_i <- a_i is stored in self.
                ...
                /// Return `true` if all the elements of the vector are equal to 0.
                #[doc(alias = $name _isnull)]
                pub fn is_null(&self) -> bool {
                    unsafe { sys::[<$name _isnull>](self.unwrap_shared()) == 1 }
                }

                /// Return `true` if all the elements of the vector are
                /// stricly positive.
                ...
                /// These functions return a vector view of a subvector of
                /// another vector v. The start of the new vector is offset by
                /// offset elements from the start of the original vector. The
                /// new vector has n elements. Mathematically, the i-th
                ...
                ///
                /// The data pointer of the returned vector struct is set to
                /// null if the combined parameters (offset,n) overrun the
                /// end of the original vector.
                ///
                /// The new vector is only a view of the block underlying the
                /// original vector, v. The block containing the elements
                /// of v is not owned by the new vector. When the view
                /// goes out of scope the original vector v and its block
                /// will continue to exist. The original memory can only
                /// be deallocated by freeing the original vector. Of
                /// course, the original vector should not be deallocated
                /// while the view is still in use.
                ///
                /// The function gsl_vector_const_subvector is equivalent to
                /// gsl_vector_subvector but can be used for vectors which
                /// are declared const.
                // checker:ignore
                ...
                pub fn slice(
                    ...
                    unsafe {
                        Self::view_mut(sys::[<$name _subvector>](
                            v.unwrap_unique(), offset, n))
                    }
                }

                /// These functions return a vector view of a subvector of
                /// another vector v with an additional stride
                /// argument. The subvector is formed in the same way as
                /// for gsl_vector_subvector but the new vector has n
                /// elements with a step-size of stride from one element
                /// to the next in the original vector. Mathematically,
                /// the i-th element of the new vector v’ is given by,
                ///
                /// v'(i) = v->data[(offset + i*stride)*v->stride]
                /// where the index i runs from 0 to n-1.
                ///
                /// Note that subvector views give direct access to the
                /// underlying elements of the original vector. For
                /// example, the following code will zero the even
                /// elements of the vector v of length n, while leaving
                ...
                /// ```
                /// A vector view can be passed to any subroutine which takes
                /// a vector argument just as a directly allocated vector
                /// would be, using &view.vector.  For example, the
                /// following code computes the norm of the odd elements
                /// of v using the BLAS routine DNRM2,
                ///
                /// ```C
                ...
                /// ```
                /// The function gsl_vector_const_subvector_with_stride is
                /// equivalent to gsl_vector_subvector_with_stride but can
                /// be used for vectors which are declared const.
                // checker:ignore
                ...
                pub fn slice_with_stride(
                    ...
                    unsafe {
                        Self::view_mut(sys::[<$name _subvector_with_stride>](
                            v.vec,
                            offset,
                            stride,
                            n))
                ...
                /// These functions return a vector view of an array. The
                /// start of the new vector is given by base and has n
                /// elements. Mathematically, the i-th element of the new
                ...
                /// where the index i runs from 0 to n-1.
                ///
                /// The array containing the elements of v is not owned by the
                /// new vector view. When the view goes out of scope the
                /// original array will continue to exist. The original
                /// memory can only be deallocated by freeing the original
                /// pointer base. Of course, the original array should not
                /// be deallocated while the view is still in use.
                ///
                /// The function gsl_vector_const_view_array is equivalent to
                /// gsl_vector_view_array but can be used for arrays which
                /// are declared const.
                // checker:ignore
                #[doc(alias = $name _view_array)]
                pub fn from_array(base: &mut [f64]) -> ViewMut<'_, Self> {
                    unsafe {
                        Self::view_mut(sys::[<$name _view_array>](
                            base.as_mut_ptr() as _, base.len() as _))
                    }
                }

                /// These functions return a vector view of an array base with
                /// an additional stride argument.  The subvector is
                /// formed in the same way as for gsl_vector_view_array
                /// but the new vector has n elements with a step-size of
                /// stride from one element to the next in the original
                /// array. Mathematically, the i-th element of the new
                /// vector v’ is given by,
                ///
                /// v'(i) = base\[i*stride\]
                ///
                /// where the index i runs from 0 to n-1.
                ///
                /// Note that the view gives direct access to the underlying
                /// elements of the original array. A vector view can be
                /// passed to any subroutine which takes a vector argument
                /// just as a directly allocated vector would be, using
                /// &view.vector.
                ///
                /// The function gsl_vector_const_view_array_with_stride is
                /// equivalent to gsl_vector_view_array_with_stride but
                /// can be used for arrays which are declared const.
                // checker:ignore
                #[doc(alias = $name _view_array_with_stride)]
                pub fn from_array_with_stride(
                    base: &mut [$rust_ty],
                    stride: usize
                ) -> ViewMut<'_, Self> {
                    unsafe {
                        Self::view_mut(sys::[<$name _view_array_with_stride>](
                            base.as_mut_ptr(),
                            stride,
                            base.len() as _))
                    }
...
gsl_vec_complex!(VecC32, gsl_vector_complex_float, C32, f32);
type C64 = Complex<f64>;
gsl_vec_complex!(VecC64, gsl_vector_complex, C64, f64);
