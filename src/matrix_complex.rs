...
macro_rules! gsl_matrix_complex {
    ($rust_name:ident, $name:ident, $complex: ty, $complex_c:ident) => {
        ...
        use crate::vector_complex::[<Vec $complex>];
        ...
        impl $rust_name {
            ...
            pub fn new(n1: usize, n2: usize) -> Self {
                ...
                if tmp.is_null() {
                    panic!("{}::new cannot allocate memory", stringify!($rust_name));
                }
                Self::wrap(tmp)
            }
            ...
            pub fn get_row(&self, y: usize) -> Result<[<Vec $complex>], Error> {
            ...
            pub fn get_col(&self, x: usize) -> Result<[<Vec $complex>], Error> {
            ...
            pub fn set_row(&mut self, y: usize, v: &[<Vec $complex>]) -> Result<(), Error> {
                let ret = unsafe {
            ...
            pub fn set_col(&mut self, x: usize, v: & [<Vec $complex>]) -> Result<(), Error> {
                let ret = unsafe {
            ...
            #[doc(alias = $name _row)]
            pub fn row(&mut self, i: usize) -> ViewMut<'_, [<Vec $complex>]> {
                unsafe {
                    [<Vec $complex>]::view_mut(sys::[<$name _row>](
                        self.unwrap_unique(), i))
                }
            }

            #[doc(alias = $name _column)]
            pub fn column(&mut self, j: usize) -> ViewMut<'_, [<Vec $complex>]> {
                unsafe {
                    [<Vec $complex>]::view_mut(sys::[<$name _column>](
                        self.unwrap_unique(), j))
                }
            }

            #[doc(alias = $name _diagonal)]
            pub fn diagonal(&mut self) -> ViewMut<'_, [<Vec $complex>]> {
                unsafe {
                    [<Vec $complex>]::view_mut(sys::[<$name _diagonal>](
                        self.unwrap_unique()))
                }
            }

            #[doc(alias = $name _subdiagonal)]
            pub fn subdiagonal(&mut self, k: usize) -> ViewMut<'_, [<Vec $complex>]> {
                unsafe {
                    [<Vec $complex>]::view_mut(sys::[<$name _subdiagonal>](
                        self.unwrap_unique(), k))
                }
            }

            #[doc(alias = $name _superdiagonal)]
            pub fn superdiagonal(&mut self, k: usize) -> ViewMut<'_, [<Vec $complex>]> {
                unsafe {
                    [<Vec $complex>]::view_mut(sys::[<$name _superdiagonal>](
                        self.unwrap_unique(), k))
                }
            }
            ...
            pub fn subrow(
                ...
                offset: usize,
                n: usize,
            ) -> ViewMut<'_, [<Vec $complex>]> {
                unsafe {
                    [<Vec $complex>]::view_mut(sys::[<$name _subrow>](
            ...
            pub fn subcolumn(
                ...
                unsafe {
                    [<Vec $complex>]::view_mut(sys::[<$name _subcolumn>](
                        self.unwrap_unique(), i, offset, n))
                }
            }
...
gsl_matrix_complex!(MatC64, gsl_matrix_complex, C64, gsl_vector_complex);
