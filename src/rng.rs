...
This module provides a large collection of random number generators
which can be accessed through a uniform interface.  Environment
variables allow you to select different generators and seeds at
runtime, so that you can easily switch between generators without
needing to recompile your program.  Each instance of a generator keeps
track of its own state, allowing the generators to be used in
multi-threaded programs.  Additional functions are available for
...
system-supplied random number generator on your computer, but you
should be aware that as computers get faster, requirements on random
number generators increase.  Nowadays, a simulation that calls a
random number generator millions of times can often finish before you
can make it down the hall to the coffee machine and back.
...
L’Ecuyer’s ftp site (see
[references](#references-and-further-reading)).  Knuth’s volume on
Seminumerical Algorithms (originally published in 1968) devotes 170
pages to random number generators, and has recently been updated in
its 3rd edition (1997).  It is brilliant, a classic.  If you don’t own
it, you should stop reading right now, run to the nearest bookstore,
and buy it.
...
values.  Of course that is just what you want for a random number
generator, but to achieve this effect, the generator must keep track
of some kind of “state” variable.  Sometimes this state is just an
integer (sometimes just the value of the previously generated random
number), but often it is more complicated than that and may involve a
whole array of numbers, possibly with some indices thrown in.  To use
the random number generators, you do not need to know the details of
what comprises the state, and besides that varies from algorithm to
algorithm.

The random number generator library uses two special structs,
[`RngType`] which holds static information about each type of
generator and [`Rng`] which describes an instance of a generator
created from a given [`RngType`].
...
- 1754 k ints/sec,    870 k doubles/sec, [`taus`](algorithms::taus)
...
function [`RngType::env_setup`].  This makes it easy try out different
...
P. L’Ecuyer, “Random Number Generation”, Chapter 4 of the Handbook on
Simulation, Jerry Banks Ed., Wiley, 1998, 93–137.
<http://www.iro.umontreal.ca/~lecuyer/papers.html>

The source code for the DIEHARD random number generator tests is also
available online,

DIEHARD source code G. Marsaglia, <http://stat.fsu.edu/pub/diehard/> A
comprehensive set of random number generator tests is available from
NIST,
...
MM&YK) available under the GNU General Public License. Thanks to
Martin Lüscher for providing notes and source code for the RANLXS and
RANLXD generators.
 */
...
impl Rng {
    /// Return a newly-created instance of a random number generator
    /// of type `T`.
    ///
    ...
    ///
    /// ```
    /// use rgsl::rng::{Rng, algorithms};
    /// let r = Rng::new(algorithms::taus());
    /// ```
    ///
    /// The generator is automatically initialized with the default
    /// seed, [`Rng::default_seed`].  This is zero by default but can
    /// be changed either directly or by using the environment
    /// variable `GSL_RNG_SEED` (see [`Random number environment
    /// variables`](https://www.gnu.org/software/gsl/manual/html_node/Random-number-environment-variables.html#Random-number-environment-variables)).
    ///
    /// # Panic
    /// Panic if there is insufficient memory to create the generator.
    #[doc(alias = "gsl_rng_alloc")]
    pub fn new(T: RngType) -> Rng {
        let tmp = unsafe { sys::gsl_rng_alloc(T.unwrap_shared()) };

        if tmp.is_null() {
            panic!("rgsl::rng::Rng::new: out of memory")
        }
        Rng::wrap(tmp)
    }

    /// Initialize (or ‘seeds’) the random number generator.
    ///
    ...
    /// different values of s >= 1 are supplied, then the generated
    /// streams of random numbers should be completely different. If
    /// the seed s is zero then the standard seed from the original
    /// implementation is used instead.  For example, the original
    /// Fortran source code for the ranlux generator used a seed of
    /// 314159265, and so choosing s equal to zero reproduces this
    /// when using gsl_rng_ranlux.
    ///
    ...
    /// setting.
    ///
    /// Note that the most generators only accept 32-bit seeds, with
    /// higher values being reduced modulo 2^32. For generators with
    /// smaller ranges the maximum seed value will typically be lower.
    ...
    /// auxiliary functions [`Self::max`] and [`Self::min`].
    ...
    ///
    /// The range includes 0.0 but excludes 1.0.  The value is
    /// typically obtained by dividing the result of
    /// `self.`[`get()`](Self::get) by `self.max() + 1.0` in double
    /// precision.  Some generators compute this ratio internally so
    /// that they can provide floating point numbers with more than 32
    /// bits of randomness (the maximum number of bits that can be
    ...
    /// uniformly distributed in the range $(0,1)$, excluding both 0.0
    /// and 1.0.
    ///
    /// The number is obtained by sampling the generator with the
    /// algorithm of [`Self::uniform`] until a non-zero value is
    ...
    /// Return a random integer from 0 to n-1 inclusive by scaling
    /// down and/or discarding samples from the generator.
    ///
    ...
    /// smaller than the range of the underlying generator. The
    /// parameter n must be less than or equal to the range of the
    /// generator r.  If n is larger than the range of the generator
    /// then the function calls the error handler with an error code
    /// of GSL_EINVAL and returns zero.
    ///
    /// In particular, this function is not intended for generating
    /// the full range of unsigned integer values $\[0, 2^{32}-1\]$.
    /// Instead choose a generator with the maximal integer range and
    /// zero minimum value, such as [`ranlxd1`](algorithms::ranlxd1),
    /// [`mt19937`](algorithms::mt19937) or
    /// [`taus`](algorithms::taus), and sample it directly using
    /// [`Self::get`].  The range of each generator can be found using
    ...
    /// Return the name of the generator.  For example,
    ///
    /// ```
    /// use rgsl::rng::{Rng, algorithms};
    /// let r = Rng::new(algorithms::taus());
    /// println!("r is a '{}' generator", r.name());
    /// ```
    ///
    ...
    /// Return the largest value that the get function can return.
    #[doc(alias = "gsl_rng_max")]
    pub fn max(&self) -> usize {
        unsafe { sys::gsl_rng_max(self.unwrap_shared()) as _ }
    }

    /// Return the smallest value that [`Self::get`] can return.
    /// Usually this value is zero.  There are some generators with
    /// algorithms that cannot return zero, and for these generators
    ...
    /// Return a pointer to the state of generator r. You can use this information
    ...
    /// Return a pointer to the state of generator. You can use this
    /// information to access the state directly. For example, the
    /// following code will write the state of a generator to a
    /// stream,
    ///
    /// ```C
    /// void * state = gsl_rng_state (r);
    ...
    /// Copy the random number generator `self` into the pre-existing
    /// generator `other`, making `other` into an exact copy of
    /// `self`.  The two generators must be of the same type.
    ...
    /// Return the size of the state of generator.  You can use this
    /// information to access the state directly.  For example, the
    /// following code will write the state of a generator to a
    /// stream,
    ///
    ...
    /// Randomly shuffle the order of elements in `base`.  The output
    /// of the random number generator `self` is used to produce the
    /// permutation.  The algorithm generates all possible $n!$
    /// permutations with equal probability (where $n =$`base.len()`),
    ...
    /// Fill the array `dest` with objects taken randomly from the
    /// elements of the array `src`.  The output of the random number
    /// generator `self` is used to make the selection.  The algorithm
    /// ensures all possible samples are equally likely, assuming a
    /// perfect source of randomness.
    ///
    /// The objects are sampled without replacement, thus each object
    /// can only appear once in `dest`.  It is required that
    /// `dest.len()` be less than or equal to `src.len()`.  The
    /// objects in `dest` will be in the same relative order as those
    /// in `src`.  You will need to call [`Self::shuffle`] if you want
    /// to randomize the order.
    ///
    /// The following code shows how to select a random sample of
    /// three unique numbers from the set 0 to 99,
    ///
    /// ```C
    /// double a[3], b[100];
    ///
    ...
    /// This function is like [`Self::choose`] but samples items from
    /// the original array items `src` with replacement, so the same
    /// object can appear more than once in the output sequence
    /// dest. There is no requirement that `dest.len()` be less than
    /// `src.len()` in this case.
    ...
    /// Compute a random sample n[] from the multinomial distribution
    /// formed by N trials from an underlying distribution `p`.  The
    /// distribution function for `n[]` is,
    ///
    /// $$P(n₁, n₂, ..., n_K) =
    ///   (N!/(n₁! n₂! ... n_K!)) p₁^{n₁} p₂^{n₂} ... p_K^{n_K}$$
    ///
    /// where $K =$`p.len()`, $(n₁, n₂, …, n_K)$ are nonnegative
    /// integers with $∑_{k=1}^K n_k = N$, and $(p₁, p₂, …, p_K)$ is a
    /// probability distribution with $∑ pᵢ = 1$.  If the slice `p` is
    /// not normalized then its entries will be treated as weights and
    /// normalized appropriately. The arrays `n[]` and `p[]` must both
    /// be of length K.
    ///
    ...
    /// Return an array of K random variates from a Dirichlet
    /// distribution of order K-1. The distribution function is
    ///
    /// $$p(θ₁,…, θ_K) dθ₁ … dθ_K =
    /// (1/Z) ∏_{i=1}^K θᵢ^{αᵢ-1} δ\Bigl(1 - ∑\_{i=1}^K θᵢ\Bigr) dθ₁ … dθ_K$$
    ///
    /// for $θᵢ ≥ 0$ and $αᵢ > 0$.  The delta function ensures that $∑
    /// θᵢ = 1$.  The normalization factor Z is
    ///
    /// $$Z = {∏_{i=1}^K Γ(αᵢ)} \Bigm/ {Γ\Bigl(∑\_{i=1}^K αᵢ\Bigr)}.$$
    ///
    /// The random variates are generated by sampling K values from
    /// gamma distributions with parameters a=alpha_i, b=1, and
    /// renormalizing.  See A.M. Law, W.D. Kelton, Simulation Modeling
    /// and Analysis (1991).
    ...
    /// Return either 0 or 1, the result of a Bernoulli trial with
    /// probability `p`.  The probability distribution for a Bernoulli
    /// trial is,
    ///
    /// $$\begin{aligned}
    /// p(0) &= 1 - p\cr
    ...
    /// Return a random variate from the beta distribution.  The
    /// distribution function is,
    ///
    /// $$p(x) dx = \frac{Γ(a+b)}{Γ(a) Γ(b)} x^{a-1} (1-x)^{b-1} dx$$
    ///
    /// for $0 ≤ x ≤ 1$.
    ...
    /// Return a random integer from the binomial distribution, the
    ...
    /// Generate a pair of correlated Gaussian variates, with mean
    /// zero, correlation coefficient `rho` and standard deviations
    /// `sigma_x` and `sigma_y` in the $x$ and $y$ directions.  The
    /// probability distribution for bivariate Gaussian random
    /// variates is,
    ///
    /// $$p(x,y) dx dy = \frac{1}{2 π σ_x σ_y √{1-ρ²}}
    /// \exp\Bigl(- \frac{x²/σ²_x + y²/σ²_y - 2 ρ x y/(σ_x σ_y)}{
    /// 2(1-ρ²)} \Bigr) dx dy$$
    ///
    /// for $x,y$ in the range $-∞$ to $+∞$.  The correlation
...
impl RngType {
    ...
    /// Return an array of all the available generator types.
    ///
    /// The function should be called once at the start of the
    /// program, if needed.  The following code fragment shows how to
    /// iterate over the array of generator types to print the names
    ...
    /// library variables [`RngType::default`] and
    /// [`Rng::default_seed`].  These global variables are defined as
    /// follows,
    ///
    /// ```C
    ...
    /// ```
    ///
    /// The environment variable `GSL_RNG_TYPE` should be the name of
    /// a generator, such as `taus` or `mt19937`.  The environment
    /// variable `GSL_RNG_SEED` should contain the desired seed value.
    /// It is converted to an unsigned long int using the C library
    /// function strtoul.
    ///
    /// If you don’t specify a generator for `GSL_RNG_TYPE` then
    /// [`algorithms::mt19937`] is used as the default.  The initial
    /// value of [`Rng::default_seed`] is zero.  See rng example in
...
///
/// The functions in [`Rng`] make no reference to the actual algorithm
...
/// provides a large number of generators of different types,
/// including simulation quality generators, generators provided for
/// compatibility with other libraries and historical generators from
/// the past.
///
/// The following generators are recommended for use in simulation.
/// They have extremely long periods, low correlation and pass most
/// statistical tests.  For the most reliable source of uncorrelated
/// numbers, the second-generation RANLUX generators have the
/// strongest proof of randomness.
pub mod algorithms {
    use crate::types::RngType;

    /// Generator of Makoto Matsumoto and Takuji Nishimura.
    ///
    /// This is a variant of the twisted generalized feedback
    /// shift-register algorithm, and is known as the “Mersenne
    /// Twister” generator.  It has a Mersenne prime period of
    /// $2^{19937} - 1$ (about $10^{6000}$) and is equi-distributed in
    /// 623 dimensions.  It has passed the DIEHARD statistical tests.
    /// It uses 624 words of state per generator and is comparable in
    /// speed to the other generators.  The original generator used a
    /// default seed of 4357 and choosing `s` equal to zero in
    /// [`Rng::set`](super::Rng::set) reproduces this.  Later versions
    /// switched to 5489 as the default seed, you can choose this
    /// explicitly via [`Rng::set`](super::Rng::set) instead if you
    /// require it.
    ///
    ...
    /// seeding procedure published by the two authors above in 2002.
    /// The original seeding procedures could cause spurious artifacts
    /// for some seed values.
    #[doc(alias = "gsl_rng_mt19937")]
    pub fn mt19937() -> RngType {
        ffi_wrap!(gsl_rng_mt19937)
    }

    /// A second-generation version of the RANLUX algorithm of
    /// Lüscher, which produces “luxury random numbers”.
    ///
    /// This generator provides single precision output (24 bits) at
    /// three luxury levels `ranlxs0`, [`ranlxs1`] and [`ranlxs2`], in
    /// increasing order of strength.  It uses double-precision
    ...
    /// $10^{171}$.  The algorithm has mathematically proven
    /// properties and can provide truly decorrelated numbers at a
    /// known level of randomness.  The higher luxury levels provide
    /// increased decorrelation between samples as an additional
    /// safety margin.
    ///
    /// Note that the range of allowed seeds for this generator is
    /// $\[0, 2^{31}-1\]$.  Higher seed values are wrapped modulo
    /// $2^{31}$.
    ...
    /// Lüscher, which produces “luxury random numbers”.
    ///
    /// See [`ranlxs0`] for more information.
    ...
    /// A second-generation version of the RANLUX algorithm that
    /// produces double precision output (48 bits).
    ///
    /// See [`ranlxs0`] for more information.
    ...
    /// An implementation of the original algorithm developed by
    /// Lüscher.
    ///
    /// It uses a lagged-fibonacci-with-skipping algorithm to produce
    /// “luxury random numbers”. It is a 24-bit generator, originally
    /// designed for single-precision IEEE floating point
    /// numbers. This implementation is based on integer arithmetic,
    /// while the second-generation versions RANLXS and RANLXD
    /// described above provide floating-point implementations which
    /// will be faster on many platforms. The period of the generator
    /// is about 10^171. The algorithm has mathematically proven
    /// properties and it can provide truly decorrelated numbers at a
    /// known level of randomness. The default level of decorrelation
    /// recommended by Lüscher is provided by gsl_rng_ranlux, while
    /// gsl_rng_ranlux389 gives the highest level of randomness, with
    /// all 24 bits decorrelated. Both types of generator use 24 words
    /// of state per generator.
    ///
    /// For more information see,
    ///
    ...
    /// An implementation of the original algorithm developed by Lüscher.
    ///
    /// It uses a lagged-fibonacci-with-skipping algorithm to produce
    /// “luxury random numbers”. It is a 24-bit generator, originally
    /// designed for single-precision IEEE floating point
    ...
    /// A combined multiple recursive generator by L’Ecuyer.
    ///
    /// Its sequence is,
    ///
    /// $$zₙ = (xₙ - yₙ) \bmod m₁$$
    ///
    /// where the two underlying generators $xₙ$ and $yₙ$ are,
    ///
    /// $$\begin{aligned}
    /// xₙ &= (a₁ x_{n-1} + a₂ x_{n-2} + a₃ x_{n-3}) \bmod m₁ \cr
    /// yₙ &= (b₁ y_{n-1} + b₂ y_{n-2} + b₃ y_{n-3}) \bmod m₂
    /// \end{aligned}$$
    ///
    /// with coefficients $a₁ = 0$, $a₂ = 63308$, $a₃ = -183326$, $b₁
    /// = 86098$, $b₂ = 0$, $b₃ = -539608$, and moduli $m₁ = 2^{31} -
    /// 1 = 2147483647$ and $m₂ = 2145483479$.
    ///
    /// The period of this generator is $\lcm(m₁³-1, m₂³-1)$, which is
    /// approximately $2^{185}$ (about $10^{56}). It uses 6 words of
    ...
    /// A fifth-order multiple recursive generator by L’Ecuyer, Blouin
    /// and Coutre.
    ///
    /// Its sequence is,
    ///
    /// $$xₙ = (a₁ x_{n-1} + a₅ x_{n-5}) \bmod m$$
    ///
    /// with $a₁ = 107374182$, $a₂ = a₃ = a₄ = 0$, $a₅ = 104480$ and
    /// $m = 2^{31} - 1$.
    ///
    /// The period of this generator is about $10^{46}$.  It uses 5
    /// words of state per generator.  More information can be found
    ...
    /// A maximally equidistributed combined Taus generator by L’Ecuyer.
    ///
    /// The sequence is,
    ///
    /// $$xₙ = (s1ₙ ⊕ s2ₙ ⊕ s3ₙ)$$
    ...
    ///
    /// $$\begin{aligned}
    /// s1_{n+1} &= ((s1ₙ\land 4294967294)≪12) ⊕ (((s1ₙ≪13) ⊕ s1ₙ)≫19)
    /// \cr
    /// s2_{n+1} &= ((s2ₙ\land 4294967288)≪4) ⊕ (((s2ₙ≪2) ⊕ s2ₙ)≫25)
    /// \cr
    /// s3_{n+1} &= ((s3ₙ\land 4294967280)≪17) ⊕ (((s3ₙ≪3) ⊕ s3ₙ)≫11)
    /// \end{aligned}$$
    ///
    /// computed modulo $2^{32}$.  In the formulas above $⊕$
    /// denotes “exclusive-or”.  Note that the algorithm relies on the
    /// properties of 32-bit unsigned integers and has been
    /// implemented using a bitmask of `0xFFFFFFFF` to make it work on
    /// 64 bit machines.
    ///
    /// The period of this generator is $2^{88}$ (about $10^{26}$).
    /// It uses 3 words of state per generator.  For more information
    /// see,
    ///
    ...
    /// 203–213.
    ///
    /// The generator [`taus2`] uses the same algorithm as `taus` but
    ...
    /// Alike a lagged-fibonacci generator, and produces each number
    /// as an xor’d sum of four previous values.
    ///
    /// $$rₙ = r_{n-A} ⊕ r_{n-B} ⊕ r_{n-C} ⊕ r_{n-D}$$
    ///
    /// Ziff (ref below) notes that “it is now widely known” that
    /// two-tap registers (such as R250, which is described below)
    /// have serious flaws, the most obvious one being the three-point
    /// correlation that comes from the definition of the generator.
    /// Nice mathematical properties can be derived for GFSR’s, and
    /// numerics bears out the claim that 4-tap GFSR’s with
    /// appropriately chosen offsets are as random as can be measured,
    /// using the author’s test.
    ///
    /// This implementation uses the values suggested the example on
    /// p392 of Ziff’s article: $A=471$, $B=1586$, $C=6988$, $D=9689$.
    ///
    /// If the offsets are appropriately chosen (such as the one ones
    /// in this implementation), then the sequence is said to be
    /// maximal; that means that the period is 2^D - 1, where D is the
    /// longest lag. (It is one less than 2^D because it is not
    /// permitted to have all zeros in the ra[] array.) For this
    /// implementation with D=9689 that works out to about 10^2917.
    ///
    /// Note that the implementation of this generator using a 32-bit
    /// integer amounts to 32 parallel implementations of one-bit
    /// generators.  One consequence of this is that the period of
    /// this 32-bit generator is the same as for the one-bit
    /// generator. Moreover, this independence means that all 32-bit
    /// patterns are equally likely, and in particular that 0 is an
    /// allowed random value. (We are grateful to Heiko Bauke for
    /// clarifying for us these properties of GFSR random number
    /// generators.)
    ///
    /// For more information see,
    ///
...
/// # Standard Unix random number generators
///
...
/// platform.  This makes it difficult to write portable code using
/// them and so we have included the complete set of Unix generators
/// in GSL for convenience. Note that these generators don’t produce
/// high-quality randomness and aren’t suitable for work requiring
/// accurate statistics. However, if you won’t be measuring
/// statistical quantities and just want to introduce some variation
/// into your program then these generators are quite acceptable.
pub mod unix {
    use crate::types::RngType;

    /// This is the BSD rand generator.
    ///
    /// Its sequence is
    ///
    /// $$x_{n+1} = (a xₙ + c) ̱mod m$$
    ///
    /// with $a = 1103515245$, $c = 12345$ and $m = 2^{31}$.  The seed
    /// specifies the initial value, $x₁$.  The period of this
    /// generator is $2^{31}$, and it uses 1 word of storage per
    /// generator.
    ...
    /// Random family of functions, a set of linear feedback shift
    /// register generators originally used in BSD Unix.
    ///
    /// There are several versions of random in use today: the
    /// original BSD version (e.g. on SunOS4), a libc5 version (found
    /// on older GNU/Linux systems) and a glibc2 version. Each version
    /// uses a different seeding procedure, and thus produces
    /// different sequences.
    ///
    /// The original BSD routines accepted a variable length buffer
    /// for the generator state, with longer buffers providing
    /// higher-quality randomness. The random function implemented
    /// algorithms for buffer lengths of 8, 32, 64, 128 and 256 bytes,
    /// and the algorithm with the largest length that would fit into
    /// the user-supplied buffer was used. To support these algorithms
    /// additional generators are available with the following names,
    ///
    /// * gsl_rng_random8_bsd
    ...
    /// original BSD random function used a 128-byte default buffer
    /// and so gsl_rng_random_bsd has been made equivalent to
    ...
    /// The random family of functions, a set of linear feedback shift
    /// register generators originally used in BSD Unix.
    ///
    /// There are several versions of random in use today: the
    /// original BSD version (e.g. on SunOS4), a libc5 version (found
    ...
    /// This is the Unix rand48 generator.
    ///
    /// Its sequence is
    ///
    /// $$x_{n+1} = (a xₙ + c) \bmod m$$
    ///
    /// defined on 48-bit unsigned integers with $a = 25214903917$, $c
    /// = 11$ and $m = 2^{48}.  The seed specifies the upper 32 bits
    /// of the initial value, $x₁$, with the lower 16 bits set to
    /// `0x330E`.  The function [`Rng::get`](super::Rng::get) returns
    /// the upper 32 bits from each term of the sequence.  This does
    /// not have a direct parallel in the original rand48 functions,
    /// but forcing the result to type long int reproduces the output
    /// of mrand48. The function gsl_rng_uniform uses the full 48 bits
    /// of internal state to return the double precision number
    /// $xₙ/m$, which is equivalent to the function drand48.  Note
    /// that some versions of the GNU C Library contained a bug in
    /// mrand48 function which caused it to produce different results
    /// (only the lower 16-bits of the return value were set).
...
///
/// The generators in this module are provided for compatibility with
/// existing libraries. If you are converting an existing program to
/// use GSL then you can select these generators to check your new
/// implementation against the original one, using the same random
/// number generator. After verifying that your new program reproduces
/// the original results you can then switch to a higher-quality
/// generator.
///
/// Note that most of the generators in this section are based on
/// single linear congruence relations, which are the least
/// sophisticated type of generator.  In particular, linear
/// congruences have poor properties when used with a non-prime
/// modulus, as several of these routines do (e.g. with a power of two
/// modulus, 2^31 or 2^32). This leads to periodicity in the least
/// significant bits of each number, with only the higher bits having
/// any randomness.  Thus if you want to produce a random bitstream it
/// is best to avoid using the least significant bits.
pub mod other {
    use crate::types::RngType;
    ...
    /// $$x_{n+1} = (a xₙ) \bmod m$$
    ///
    /// defined on 48-bit unsigned integers with $a = 44485709377909$
    /// and $m = 2^{48}$.  The seed specifies the lower 32 bits of the
    /// initial value, $x₁1, with the lowest bit set to prevent the
    /// seed taking an even value. The upper 16 bits of $x₁$ are set
    /// to 0.  A consequence of this procedure is that the pairs of
    /// seeds 2 and 3, 4 and 5, etc. produce the same sequences.
    ///
    /// The generator compatible with the CRAY MATHLIB routine
    /// RANF. It produces double precision floating point numbers
    /// which should be identical to those from the original RANF.
    ///
    /// There is a subtlety in the implementation of the seeding. The
    /// initial state is reversed through one step, by multiplying by
    /// the modular inverse of a mod m. This is done for compatibility
    /// with the original CRAY implementation.
    ///
    /// Note that you can only seed the generator with integers up to
    /// $2^{32}$, while the original CRAY implementation uses
    /// non-portable wide integers which can cover all $2^{48}$ states
    /// of the generator.
    ///
    /// The function [`Rng::get`](super::Rng::get) returns the upper
    /// 32 bits from each term of the sequence.  The function
    /// [`Rng::uniform`](super::Rng::uniform) uses the full 48 bits to
    /// return the double precision number $xₙ/m$.
    ///
    ...
    /// The RANMAR lagged-fibonacci generator of Marsaglia, Zaman and
    /// Tsang.
    ///
    /// It is a 24-bit generator, originally designed for
    /// single-precision IEEE floating point numbers.  It was included
    /// in the CERNLIB high-energy physics library.
    #[doc(alias = "gsl_rng_ranmar")]
    pub fn ranmar() -> RngType {
        ffi_wrap!(gsl_rng_ranmar)
    }

    /// The shift-register generator of Kirkpatrick and Stoll.
    ///
    /// The sequence is based on the recurrence
    ///
    /// $$xₙ = x_{n-103} ⊕ x_{n-250}$$
    ///
    /// where $⊕$ denotes “exclusive-or”, defined on 32-bit words.
    /// The period of this generator is about $2^{250}$ and it uses
    ...
    /// development of MT19937.
    ///
    /// However, it is still an acceptable generator in its own right.
    /// It has a period of $2^{800}$ and uses 33 words of storage per
    /// generator.
    ///
    /// For more information see,
    ///
    ...
    /// The VAX generator MTH\$RANDOM.
    ///
    /// Its sequence is,
    ///
    /// $$x_{n+1} = (a xₙ + c) \bmod m$$
    ///
    /// with $a = 69069$, $c = 1$ and $m = 2^{32}.  The seed specifies
    /// the initial value, $x₁$. The period of this generator is
    /// $2^{32}$ and it uses 1 word of storage per generator.
    #[doc(alias = "gsl_rng_vax")]
    pub fn vax() -> RngType {
        ffi_wrap!(gsl_rng_vax)
    }

    /// The random number generator from the INMOS Transputer
    /// Development system.
    ///
    /// Its sequence is,
    ///
    /// $$x_{n+1} = (a xₙ) \bmod m$$
    ///
    /// with $a = 1664525$ and $m = 2^{32}$.  The seed specifies the
    /// initial value, $x₁$.
    ...
    /// The IBM RANDU generator.
    ///
    /// Its sequence is
    ///
    /// $$x_{n+1} = (a xₙ) \bmod m$$
    ///
    /// with $a = 65539$ and $m = 2^{31}$.  The seed specifies the
    /// initial value, $x₁$.  The period of this generator was only
    /// $2^{29}$.  It has become a textbook example of a poor generator.
    #[doc(alias = "gsl_rng_randu")]
    pub fn randu() -> RngType {
        ffi_wrap!(gsl_rng_randu)
    }

    /// Park and Miller’s “minimal standard” MINSTD generator.
    ///
    /// This isa simple linear congruence which takes care to avoid
    /// the major pitfalls of such algorithms.  Its sequence is,
    ///
    /// $$x_{n+1} = (a xₙ) \bmod m$$
    ///
    /// with $a = 16807$ and $m = 2^{31} - 1 = 2147483647$.  The seed
    /// specifies the initial value, $x₁$. The period of this
    /// generator is about $2^{31}$.
    ///
    /// This generator was used in the IMSL Library (subroutine RNUN)
    /// and in MATLAB (the RAND function) in the past.  It is also
    /// sometimes known by the acronym "GGL" (I'm not sure what that
    /// stands for).
    ///
    /// For more information see,
    ///
    ...
    /// A reimplementation of the 16-bit SLATEC random number
    /// generator RUNIF.
    ///
    /// A generalization of the generator to 32 bits is provided by
    /// `uni32`.  The original source code is available from NETLIB.
    #[doc(alias = "gsl_rng_uni32")]
    pub fn uni32() -> RngType {
        ffi_wrap!(gsl_rng_uni32)
    }

    /// This is the SLATEC random number generator RAND.
    ///
    /// It is ancient. The original source code is available from NETLIB.
    #[doc(alias = "gsl_rng_slatec")]
    pub fn slatec() -> RngType {
        ffi_wrap!(gsl_rng_slatec)
    }

    /// The ZUFALL lagged Fibonacci series generator of Peterson.
    ///
    /// Its sequence is,
    ///
    /// $$\begin{aligned}
    ...
    /// in Seminumerical Algorithms, 3rd Ed., page 108.
    ///
    /// Its sequence is,
    ///
    /// $$xₙ = (a₁ x_{n-1} + a₂ x_{n-2}) \bmod m$$
    ///
    /// with $a₁ = 271828183$, $a₂ = 314159269$, and $m = 2^{31} - 1$.
    ...
    /// A second-order multiple recursive generator described by Knuth
    /// in Seminumerical Algorithms, 3rd Ed., Section 3.6.
    ///
    /// Knuth provides its C code.  The updated routine `knuthran2002`
    /// is from the revised 9th printing and corrects some weaknesses
    /// in the earlier version, which is implemented as [`knuthran`].
    ...
    /// The multiplicative generator is taken from Knuth’s
    /// Seminumerical Algorithms, 3rd Ed., pages 106–108.
    ///
    /// Their sequence is,
    ///
    /// $$x_{n+1} = (a xₙ) \bmod m$$
    ///
    /// where the seed specifies the initial value, $x₁$.  The
    /// parameters $a$ and $m$ are as follows,
    /// - [Borosh-Niederreiter](borosh13): $a = 1812433253$, $m = 2^{32}$,
    /// - [Fishman18](fishman18): $a = 62089911$, $m = 2^{31} - 1$,
    /// - [Fishman20](fishman20): $a = 48271$, $m = 2^{31} - 1$,
    /// - [L’Ecuyer](lecuyer21): $a = 40692$, $m = 2^{31} - 249$,
    /// - [Waterman](waterman14): $a = 1566083941$, $m = 2^{32}$.
    ...
    /// L’Ecuyer–Fishman random number generator.
    ///
    /// It is taken from Knuth’s Seminumerical Algorithms, 3rd Ed.,
    /// page 108. Its sequence is,
    ///
    /// $$z_{n+1} = (xₙ - yₙ) \bmod m$$
    ///
    /// with $m = 2^{31} - 1$.  $xₙ$ and $yₙ$ are given by the
    /// [`fishman20`] and [`lecuyer21`] algorithms.  The seed
    /// specifies the initial value, $x₁$.
    #[doc(alias = "gsl_rng_fishman2x")]
    pub fn fishman2x() -> RngType {
        ffi_wrap!(gsl_rng_fishman2x)
    }

    /// The Coveyou random number generator.
    ///
    /// It is taken from Knuth’s Seminumerical Algorithms, 3rd Ed.,
    /// Section 3.2.2. Its sequence is,
    ///
    /// $$x_{n+1} = (xₙ (xₙ + 1)) \bmod m$
    ///
    /// with $m = 2^{32}$.  The seed specifies the initial value, $x₁$.
