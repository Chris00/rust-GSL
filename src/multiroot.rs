...
Functions for multidimensional root-finding (solving nonlinear systems
with n equations in n unknowns). The library provides low level
...
solution, with full access to the intermediate steps of the iteration.
Each class of methods uses the same framework, so that you can switch
between solvers at runtime without needing to recompile your program.
Each instance of a solver keeps track of its own state, allowing the
solvers to be used in multi-threaded programs.  The solvers are based
...
exist.  All algorithms proceed from an initial guess using a variant
of the Newton iteration,

$$x ↦ x′ = x - J^{-1} f(x)$$

where $x$, $f$ are vector quantities and $J$ is the Jacobian matrix
$J_{ij} = \partial f_i / \partial x_j$.  Additional strategies can be
used to enlarge the region of convergence.  These include requiring a
decrease in the norm $|f|$ on each step proposed by Newton’s method,
or taking steepest-descent steps in the direction of the negative
gradient of $|f|$.
...
computation of the $n^2$ terms of the matrix becomes too expensive.
For these reasons the algorithms provided by the library are divided
into two classes according to whether the derivatives are available or
not.
...
impl MultiRootFSolverType {
    ...
    /// solving a multidimensional system. It uses the Newton iteration
    ///
    /// $$x ↦ x - J^{-1} f(x)$$
    ///
    ...
    ///
    /// $$J_{ij} = (f_i(x + δ_j) - f_i(x)) / δ_j$$
    ///
    /// where $δ_j$ is a step of size $√ε |x_j|$ with $ε being the
    /// machine precision (ε ≈ 2.22 · 10^{-16}).  The order of
    /// convergence of Newton’s algorithm is quadratic, but the finite
    /// differences require $n^2$ function evaluations on each
    /// iteration.  The algorithm may become unstable if the finite
    /// differences are not a good approximation to the true
    /// derivatives.
    ...
    ///
    /// $$J^{-1} → J^{-1} - (J^{-1} df - dx) dx^T J^{-1} / dx^T J^{-1} df$$
    ///
    /// where the vectors $dx$ and $df$ are the changes in $x$ and
    /// $f$.  On the first iteration the inverse Jacobian is estimated
...
ffi_wrapper!(
    /// Multidimensional root-finding solver without derivatives.
    MultiRootFSolver<'a>,
    *mut sys::gsl_multiroot_fsolver,
    ...
    ;inner_closure: Option<Box<dyn Fn(&VecF64, &mut VecF64) -> Result<(), Error> + 'a>> => None;);

impl<'a> MultiRootFSolver<'a> {
    /// Allocate an instance of a solver of type `t` with `n` unknowns.
    ///
    /// Panic if there is insufficient memory to create the solver.
    #[doc(alias = "gsl_multiroot_fsolver_alloc")]
    pub fn new(t: &MultiRootFSolverType, n: usize) -> MultiRootFSolver<'a> {
        let ptr = unsafe { sys::gsl_multiroot_fsolver_alloc(t.unwrap_shared(), n) };

        if ptr.is_null() {
            panic!("rgsl::multiroot::MultiRootFSolver: out of memory")
        }
        MultiRootFSolver::wrap(ptr)
    }

    /// Initialize, or reinitialize, an existing solver `self` to use
    /// the multi function `f` with `n` unknowns.
    #[doc(alias = "gsl_multiroot_fsolver_set")]
    pub fn set<F: Fn(&VecF64, &mut VecF64) -> Result<(), Error> + 'a>(
        &mut self,
        f: F,
        n: usize,
        x: &VecF64,
    ...
    /// encounters an unexpected problem then an error will be returned:
    ///
    /// - [`Error::BadFunction`] the iteration encountered a singular point
    ///   where the function evaluated to Inf or NaN.
...
impl MultiRootFdfSolverType {
    ...
    /// written by Jorge J. Moré, Burton S. Garbow and Kenneth
    /// E. Hillstrom.  The Hybrid algorithm retains the fast
    /// convergence of Newton’s method but will also reduce the
    ...
    /// position $x′$ must satisfy the condition $|D (x′ - x)| < δ$,
    /// where $D$ is a diagonal scaling matrix and $δ$ is the size of
    /// the trust region.  The components of D are computed
    /// internally, using the column norms of the Jacobian to estimate
    /// the sensitivity of the residual to each component of $x$.
    /// This improves the behavior of the algorithm for badly scaled
    /// functions.
    ///
    ...
    /// inside the trust region, $dx = - α J^{-1} f(x) - β ∇|f(x)|²$.
    ...
    /// the resulting point, $x′$.  If the step reduces the norm of
    /// the function sufficiently then it is accepted and size of the
    ...
    /// steps fail to make any improvement, [`Error::NoProgress`] the
    /// iteration is not making any progress, preventing the algorithm
    /// from continuing.  [`Error::NoProgressJacobian`] re-evaluations
    /// of the Jacobian indicate that the iteration is not making any
    ...
    /// This algorithm is an unscaled version of [`Self::hybridsj`].
    /// The steps are controlled by a spherical trust region $|x' - x|
    /// < δ$, instead of a generalized region.  This can be useful if
    /// the generalized region estimated by [`Self::hybridsj`] is
    /// inappropriate.
    ...
    /// the following sequence, $x \to x′ = x - J^{-1} f(x)$ where the
    /// Jacobian matrix $J$ is computed from the derivative functions
    /// provided by $f$.  The step dx is obtained by solving the
    /// linear system, $J dx = - f(x)$ using LU decomposition.  If the
    /// Jacobian matrix is singular, the error [`Error::Domain`] is
    /// returned.
    ...
    /// step of relative size, $t = (\sqrt{1 + 6 r} - 1) / (3 r)$ is
    /// proposed, with $r$ being the ratio of norms $|f(x')|²/|f(x)|²$.
    /// This procedure is repeated until a suitable step size is
    /// found.
...
pub struct MultiRootFdfSolverFunction<'a> {
    pub f: Box<dyn Fn(&VecF64, &mut VecF64) -> Result<(), Error> + 'a>,
    pub df: Box<dyn Fn(&VecF64, &mut MatF64) -> Result<(), Error> + 'a>,
    pub fdf: Box<dyn Fn(&VecF64, &mut VecF64, &mut MatF64) -> Result<(), Error> + 'a>,
    pub n: usize,
...
impl<'a> MultiRootFdfSolverFunction<'a> {
    ...
    pub fn new<
        F: Fn(&VecF64, &mut VecF64) -> Result<(), Error> + 'a,
        DF: Fn(&VecF64, &mut MatF64) -> Result<(), Error> + 'a,
        FDF: Fn(&VecF64, &mut VecF64, &mut MatF64) -> Result<(), Error> + 'a,
    >(
        f: F,
        df: DF,
        fdf: FDF,
...
impl MultiRootFdfSolver {
    /// Allocate an instance of a derivative solver of type `t` for a
    ...
    /// Set, or reset, an existing solver to use the functions `f`,
    /// and the initial guess `x`.
    #[doc(alias = "gsl_multiroot_fdfsolver_set")]
    pub fn set(&mut self, f: &mut MultiRootFdfSolverFunction, x: &VecF64) -> Result<(), Error> {
        let ret = unsafe {
    ...
    /// encounters an unexpected problem then an error will be
    /// returned,
    ///
    /// - [`Error::BadFunction`] the iteration encountered a singular
    ///   point where the function or its derivative evaluated to Inf
    ///   or NaN;
    /// - [`Error::NoProgress`] the iteration is not making any
    ///   progress, preventing the algorithm from continuing.
    ///
    /// The solver maintains a current best estimate of the root and
    /// its function value at all times.  This information can be
    /// accessed with [`Self::root`], [`Self::f`], and [`Self::dx`]
    /// methods.
...
mod tests {
    ...
    /// ```compile_fail
    /// use crate::rgsl::*;
    /// use crate::rgsl::multiroot::{MultiRootFSolver, MultiRootFSolverType};
    ///
    ...
    ///         Ok(())
    ///     },
    ///     2, &VecF64::from_slice(&[-10.0, 1.0]));
    /// }
    ///
    /// let mut root = MultiRootFSolver::new(&MultiRootFSolverType::hybrid(), 2);
    /// set(&mut root);
    ...
    /// Checking a test function must return a success criteria (or
    /// failure).
    fn rosenbrock_f(x: &VecF64, f: &mut VecF64) -> Result<(), Error> {
        f.set(0, RPARAMS.0 * (1.0 - x.get(0)));
        f.set(1, RPARAMS.1 * (x.get(1) - x.get(0).powf(2.0)));
        Ok(())
    }

    fn rosenbrock_df(x: &VecF64, J: &mut MatF64) -> Result<(), Error> {
        J.set(0, 0, -RPARAMS.0);
        J.set(0, 1, 0f64);
    ...
    fn rosenbrock_fdf(x: &VecF64, f: &mut VecF64, J: &mut MatF64) -> Result<(), Error> {
        rosenbrock_f(x, f)?;
        rosenbrock_df(x, J)?;
        Ok(())
    }
    ...
    fn test_multiroot_fsolver() {
        // setup workspace
        let mut multi_root = MultiRootFSolver::new(&MultiRootFSolverType::hybrid(), 2);
        let array_size: usize = 2;
        let guess_value = VecF64::from_slice(&[-10.0, -5.0]);
        multi_root
